...
- **Markdowns are cosmetic.** An item's optional `wasPrice` (and a category's `wasLotPrice` / the bundle's `wasPrice`) renders as a struck-through old price on the storefront; pricing always uses `price` / `lotPrice`.
